# Claude Code Project Instructions — ed25519-dalek-kotlin

## Project Overview

This is **ed25519-dalek-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`ed25519-dalek`](https://crates.io/crates/ed25519-dalek).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/ed25519-dalek/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/ed25519-dalek>
